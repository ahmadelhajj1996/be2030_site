  ...
  },
  {
    ...
    render: (item) => (
      <div className="flex gap-x-8">
        <button
          onClick={() => navigate(`/parts/${item.post_id}`)}
          aria-label="حذف"
...
function Parts() {
  ...
  const { data, isFetched, isLoading, isError, error } = useGet(
    ...
    {
      staleTime: 5 * 60 * 1000,
      select: (data) => data.filter((e) => e.post_id == id),
    }
  );
  ...
  const handleDelete = useCallback(() => {
    ...
    deleteItem({ url: `parts/${currentItem.id}` });
    setModal("");
    setCurrentItem(null);
  ...
  const handleAddPost = (values, { setSubmitting, resetForm }) => {
    ...
    formData.append("post_id", id); // Use the id from URL params
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h2 className="font-normal text-lg mb-4">إضافة جزء جديد</h2>
